...
// Operations
	...
	void RemoveAll(bool keephash=false);
...
template<class KEY, class ARG_KEY, class VALUE, class ARG_VALUE>
void CMap<KEY, ARG_KEY, VALUE, ARG_VALUE>::RemoveAll(bool keephash)
{
	ASSERT_VALID(this);
...
CMap<KEY, ARG_KEY, VALUE, ARG_VALUE>::NewAssoc(ARG_KEY key)
{
	...
	m_nCount++;
	ASSERT(m_nCount > 0);  // make sure we don't overflow
  //setting memory to 0 before calling constructors so basic variables will be
  //initialized
